...
#Константы
#=======================
g = 9.8066 # Ускорение свободного падения
...
koef = 101325/760 # Коэффициент перевода мм рт. ст. в Па5
#=======================
...
#Учет главного коэффициента кода
beta = fiz['beta']
...
finrk(x, y, z, y_max, t_ymax, vx, vy, vz, g, dt, beta, inp, wxz, next_target, last_recorder, times, heights, speed, Kin, Pot, Meh, vg, vv, lp, Fsv, lz, vp)
rk=finrk(x, y, z, y_max, t_ymax, vx, vy, vz, g, dt, beta, inp, wxz, next_target, last_recorder, times, heights, speed, Kin, Pot, Meh, vg, vv, lp, Fsv, lz, vp)


y_max=rk['y_max']
t_ymax=rk['t_ymax']
vx_last = rk['vx'] #Скорость перед падением по оси Х
vy_last = rk['vy'] #Скорость перед падением по оси У
vz_last = rk['vz'] #Скорость перед падением по оси Z
v_last = math.sqrt(rk['vx']**2+rk['vy']**2+rk['vz']**2) #Модуль реальной скорости перед падением
a_last = abs(math.degrees(math.atan2(rk['vy'], rk['vx']))) #Угол к горизонту в момент падения
